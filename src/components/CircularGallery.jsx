...
function createTextTexture(gl, text, font = 'bold 30px "League Spartan", sans-serif', color = "#ffffff") {
...
// ---------- Title (plane avec texture de texte) ----------
class Title {
  constructor({ gl, plane, text, textColor = "#545050", font = '600 28px "League Spartan", sans-serif' }) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
    this.text = text;
  ...
  createMesh() {
    ...
    // positionné sous la carte
    this.mesh.position.y = -this.plane.scale.y * 0.5 - textHeight * 0.5 - 0.05;
    this.mesh.setParent(this.plane);
  }
}

// ---------- Media (carte image + titre) ----------
class Media {
  constructor({
    geometry,
    gl,
...
class App {
  ...
  createMedias(items, bend, textColor, borderRadius, font) {
    const galleryItems = (items && items.length ? items : []).concat(items && items.length ? items : []);
